...
export function PassportsPage() {
  ...
  const cols: Column<PassportSummary>[] = [
    { key: 'serial', header: t('passports.serial'), render: (r) => <span className="mono">{r.serial}</span>, sortValue: (r) => r.serial, card: 'title' },
    ...
    { key: 'status', header: t('supply.status'), render: (r) => <PassportStatusChip status={r.status} small />, sortValue: (r) => r.status, card: 'meta' },
