...
export function useBaseline() {
  return useQuery({ queryKey: keys.planning.baseline, queryFn: () => api.get<PlanningBaseline>('/planning/baseline') });
}
export function usePresets() {
  return useQuery({ queryKey: keys.planning.presets, queryFn: () => api.get<ScenarioPreset[]>('/planning/scenarios/presets'), staleTime: Infinity });
}
export function useScenarios() {
  return useQuery({ queryKey: keys.planning.scenarios, queryFn: () => api.get<Paged<PlanningScenarioSummary>>('/planning/scenarios') });
}
...
export function useCreateScenario() {
  ...
  return useMutation({
    ...
    onSuccess: () => void qc.invalidateQueries({ queryKey: keys.planning.scenarios }),
  });
}
export function useRunScenario() {
  ...
  return useMutation({
    ...
    onSuccess: (_r, id) => {
      ...
      void qc.invalidateQueries({ queryKey: keys.planning.scenarios });
    },
  });
}
